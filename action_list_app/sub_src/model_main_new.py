...
# model_main_new.pyはmodel_main.pyから変更して予測などを追加（まだうまく動いていない）
...
def result():
    ...
    if not load_model:
        ...
        # 予測
        r, date, next_action_list, next_action, ret = evaluate(flag='pred', model=best_model, data_provider=data_provider('pred', src_len, tgt_len, batch_size), criterion=criterion)
        print(r)
    ...
    def __read_data__(self):
        ...
        if not self.set_type == 3:
            ...
            if not multi_data:
                ...
                # 1~66
                border1s = [2, 41, 56] # [train0, val0, test0] # test0~test1=24
                border2s = [40, 60, 65]          # [train1, val1, test1]
            ...
            df_src = pd.read_csv("./input/src.csv",sep=",")
            data = df_src[['date']].values
            self.data = data
...
def evaluate(flag, model, data_provider, criterion):
    ...
    print("data loader : ", data_provider)
    try:
        model.eval()
        total_loss = []
    ...
    if flag=='pred':
        for src in data_provider:
            ...
            for i in range(seq_len_src - 1): # tgtの長さ分だけループ
        ...
        print("pred  : ", pred.detach().numpy())
        # plt.plot(true.squeeze().cpu().detach().numpy(), label='true')
        plt.plot(pred.squeeze().cpu().detach().numpy(), label='pre-test')
        ...
        "main__this.py ver."
        # index = df["date"][-(seq_len_src)-2:-(seq_len_tgt):1] # -37 ~ -11
        # index2 = df["date"][-(seq_len_src)-1:-2:1] # -11 ~ の予測
        # plt.plot(index, pred.squeeze().cpu().detach().numpy()[-(seq_len_src)-1:-(seq_len_tgt-1):1], label='src(input)') # 'true') # , color="blue") # true.detach())
        # plt.plot(index2, pred.squeeze().cpu().detach().numpy()[-(seq_len_tgt):-1:1], label='pred(output)')
        # index_tgt = df["date"][-(seq_len_tgt)-1:-1:1] # target
        # plt.plot(index_tgt, true.squeeze().cpu().detach().numpy()[-(seq_len_tgt)-1:-1:1], label='tgt(true)', alpha=0.5) # , color="green") # true.detach())
