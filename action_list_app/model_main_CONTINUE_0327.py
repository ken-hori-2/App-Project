...
def evaluate(flag, model, data_provider, criterion):
    ...
    if flag=='test':
        ...
        pred_date_and_actions = pd.concat([pred_date, Actions], axis=1)
        # date = pd.DataFrame(date, Actions)
        print(pred_date_and_actions)

        pred_date_and_actions.to_csv("pred_date_actions.csv")
        ...
        "***** add 2024/03/04 *****"
        # 本来はここに一週間分や一日分の推論結果を指定するが、今回はダミーデータを推論結果として出力されたとして扱う
